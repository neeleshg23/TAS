...
class BasicBlock_AMM:
    ...
    def __init__(self, in_channels, out_channels, stride=1, ncodebooks=[2]*3, kcentroids=[256]*3, weights=None, means_vars=None):
    ...
    def forward_amm(self, x):
        ...
        out, est = self.conv2d_amm(relu, self.conv2_weights, np.zeros(self.conv2_weights.shape[0]), 1, pad=1)
        block_est.append(est)
        bn2 = self.batch_norm(out, self.bn2_weights, self.bn2_bias, self.bn2_mean, self.bn2_var)
    ...
    def forward_eval(self, block_est, x):
        ...
        est = block_est.pop(0)
        out = self.conv2d_eval(est, relu, self.conv2_weights, np.zeros(self.conv2_weights.shape[0]), 1, pad=1)
        bn2 = self.batch_norm(out, self.bn2_weights, self.bn2_bias, self.bn2_mean, self.bn2_var)
...
class ResNet_AMM:
    def __init__(self, block, num_blocks, state_dict, ncodebook=[1]*22, kcentroid=[4]*22):
    ...
    # def forward_train_2(self, x, target):
    ...
    def forward_train(self, x, target):
        ...
        for i, layer_group in enumerate(layers):
            for j, layer in enumerate(layer_group):
                idx = i*len(layer_group) + j + 1
                out, [conv1, conv2], block_est = layer.forward_amm(out)
                self.amm_estimators.append(block_est)
                intermediate_results.append(conv1)
                if j == 0: 
                    intermediate_results.append(conv2)
                intermediate_results.append(out)
    ...
    def forward_eval(self, x):
        ...
        for i, layer_group in enumerate(layers):
            for j, layer in enumerate(layer_group):
                block_ests = self.amm_queue.pop(0)
                out, [conv1, conv2] = layer.forward_eval(block_ests, out)
                intermediate_results.append(conv1)
                if j == 0:
                    intermediate_results.append(conv2)
                intermediate_results.append(out)
